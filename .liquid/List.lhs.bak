...
\begin{code}
{-@ length :: xs : List v -> {v:Nat | v == size xs} @-}
length            :: List a -> Int
length Emp        = 0
...
\begin{code}
{-@ append :: as:List a -> bs:List a ->  {cs:List a  | size cs == ((size as) + (size bs))} @-}
append Emp        bs = bs
append (a :+: as) bs = a :+: (as `append` bs)

{-@ measure sizeN :: List (List a) -> Int
    sizeN Emp          = 0
    sizeN ((:+:) a as) = size a + sizeN as @-}

{-@ concat :: as : List (List a) -> ListN a (sizeN as) @-}
concat        Emp = Emp
concat (a :+: as) = a `append` concat as
